...
data:
  ...
  code: "use std::marker::PhantomData;\n\nuse algebraic_traits::{Algebraic, Associative,\
    ...
    \ Recip};\n\npub struct MulOperator<T: Numeric>(PhantomData<fn() -> T>);\n\nimpl<T:\
    \ Numeric> Algebraic for MulOperator<T> {\n    type Value = T;\n}\n\nimpl<T: Numeric>\
    \ Magma for MulOperator<T> {\n    fn op(a: &T, b: &T) -> T {\n        *a * *b\n\
    ...
    \ + Recip> Invertive for MulOperator<T> {\n    fn inv(x: &T) -> T {\n        x.recip()\n\
...
layout: document
redirect_from:
- /library/crates/algebra/algebraic_structure/src/mul.rs
- /library/crates/algebra/algebraic_structure/src/mul.rs.html
