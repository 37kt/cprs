...
data:
  ...
  code: "use std::marker::PhantomData;\n\nuse algebraic_traits::Act;\nuse numeric_traits::Numeric;\n\
    ...
    \ Act for CountsumAffineOperator<T> {\n    type Operand = CountSumOperator<T>;\n\
    \    type Operator = AffineOperator<T>;\n\n    fn act(&CountSum { count, sum }:\
    \ &CountSum<T>, &Affine(a, b): &Affine<T>) -> CountSum<T> {\n        CountSum\
    \ {\n            count,\n            sum: a * sum + b * count,\n        }\n  \
    \  }\n}\n"
  dependsOn:
...
documentation_of: crates/algebra/algebraic_structure/src/countsum_affine.rs
layout: document
redirect_from:
- /library/crates/algebra/algebraic_structure/src/countsum_affine.rs
- /library/crates/algebra/algebraic_structure/src/countsum_affine.rs.html
