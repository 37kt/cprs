...
data:
  ...
  code: "// verification-helper: PROBLEM https://judge.yosupo.jp/problem/lcm_convolution\n\
    ...
    \ as Mint;\n\n#[fastout]\nfn main() {\n    input! {\n        n: usize,\n     \
    \   mut a: [Mint; n],\n        mut b: [Mint; n],\n    }\n    a.insert(0, 0.into());\n\
    \    b.insert(0, 0.into());\n    let c = lcm_convolution::<AddMulOperator<_>>(&a,\
    \ &b);\n    for i in 1..c.len() {\n        print!(\"{}\", c[i]);\n        print!(\"\
    {}\", if i == c.len() - 1 { \"\\n\" } else { \" \" });\n    }\n}\n"
  dependsOn:
  ...
  verifiedWith: []
documentation_of: verify/library_checker/convolution/lcm_convolution/src/main.rs
layout: document
redirect_from:
- /verify/verify/library_checker/convolution/lcm_convolution/src/main.rs
- /verify/verify/library_checker/convolution/lcm_convolution/src/main.rs.html
